function perc=perc_movements_imitation(subj_id)
...
for i=1:length(data.event)
    ...
    elseif strcmp(data.event(i).type,'artifact') && strcmp(data.event(i).code,'noat')==0
        if (last_mov_start>-1 && last_mov_end<0) || (last_mov_start>-1 && last_mov_end>-1 && last_mov_start<=data.event(i).latency<=last_mov_end+max_latency*data.srate)
            num_movements=num_movements+1.0;
            if (strcmp(code,'joy') && strcmp(data.event(i).movement,'J')) ||  (strcmp(code,'move') && strcmp(data.event(i).movement,'MO')) || (strcmp(code,'sad') && strcmp(data.event(i).movement,'S'))
                num_imitations=num_imitations+1.0;
            end
        end
    end
end
perc=num_imitations/num_movements*100.0;
