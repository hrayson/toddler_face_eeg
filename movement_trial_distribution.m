function movement_trial_distribution(subj_ids, movement_type, subj_dir_ext, max_latency)

[included_subjects excluded_subjects]=exclude_subjects(subj_ids, {'rereferenced.interp'}, subj_dir_ext, '');

movement_magnitude_dists=dict();
...
for s=1:length(included_subjects)
    ...
    for i=1:length(data.epoch)
        ...
        movement_mag_trials(movement_magnitude+1)=movement_mag_trials(movement_magnitude+1)+1;
    end
    for i=1:length(movement_mag_trials)
        movement_magnitude_dists(i-1)=[movement_magnitude_dists(i-1) movement_mag_trials(i)];
    end
end
...
for i=1:5
    subplot(1,5,i);
    hist(movement_magnitude_dists(i-1));
